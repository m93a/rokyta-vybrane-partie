...
Buď $X$ Banachův prostor. Prostor $X' \coloneqq \lin(X,\K)$ %\hspace{0.5em}
nazýváme topologickým duálem k $X$.
%Nechápu, proč tady byl hspace? -J
...
\begin{itemize}
    ...
    \item Topologický duál není totéž, co \uu{vektorový duál} (pouze lineární zobrazení $X \to \K$, nevyžaduje spojitost). Prvků vektorového duálu je víc (o ony „nespojité“). V konečné dimenzi pro $X$ Banachův je vektorový duál vždy topologickým duálem.
\end{itemize}
\end{remark}
...
\begin{corollary}
Zobrazení $\map T: x \mapsto f$ je izometrický izomorfismus (zachovává normu a je bijektivní). Proto pro všechny Hilbertovy prostory $H$ můžeme provést ztotožnění $H' \simeq H$. %TODO tento odstavec je možná špatně
\end{corollary}
...
\begin{lemma}[O linearitě a spojitosti duálního operátoru]
...
% V Rokytovi tohle není subsection
% asi by to chtělo změnit, ale nevím na co -Michal
...
\end{remark}
\begin{theorem}[O spektrálních vlastnostech samoadjungovaných operátorů] \label{4.Spektralni vlastnosti samoadjug}
...
Nechť $H$ je \uu{Hilbertův prostor} a $A$ je \uu{uzavřený} lineární podprostor v $H$. Definujeme ortogonální doplněk $A^\bot \subset H$ předpisem \begin{align*}
...
% Celkově jsme ukázali, že  $\map K(\Lambda^\bot)$ je také uzavřený podprostor v $H$ a tedy Hilbertův.
% To jsme neukázali, ne? -Michal
