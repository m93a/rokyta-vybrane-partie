...
\end{equation}
Na pravé straně rovnice máme tedy ve zdvojeném členu jakousi \uv{zpětnou vazbu}. Pouze na základě analogie s první úlohou si troufneme vyslovit následující tvrzení:
...
\end{equation}
pak je tato funkce třídy $\mathcal{C}^2 \left([0,a] \right)$ a řeší úlohu %TODO tady je možná špatně C^2 \eqref{eq:2.zadani_y(x)_vpravo}.
...
\begin{enumerate}
    ...
    \item Definujeme-li zobrazení \uv{$u\mapsto y$} z předchozího bodu a označíme-li jej $(\map{Id}- \map T )^{-1}$, platí:
    \begin{equation}
...
$a\in (0,b)$, že $|| \map T ||<1$.
Z tvrzení věty pak dostaneme \uv{existenci a jednoznačnost} řešení úlohy (\ref{eq:2.tipreseni_y(x)}), tedy i (\ref{eq:zadani_y(x)_vpravo}) na příslušném \emph{zkráceném} intervalu $[0,a]$
...
jinak řečeno, \uv{blízkým pravým stranám rovnice $u_n$ odpovídají blízká řešení}, či \uv{malé změny na pravé straně rovnice způsobují malé změny řešení}. Právě tomuto se říká \emph{stabilita řešení}.
\end{remark}
...
\Priklad{
Definujme prostor posloupností $\ell_2$
$$\ell_2\coloneqq \Big\{ \{x_n\}_{n=1}^\infty,\; x_n\in\C;\;\sum\limits_{n=1}^\infty|x_n|^2<\infty\Big\}$$

Lze ukázat, že $\ell_2$ s normou $||\{x_n\}||^2_{\ell_2}\coloneqq \sum_{n=1}^\infty|x_n|^2$ je Banachův prostor (je dokonce Hilbertův, více později). Na $\ell_2$ definujme dva tzv. \emph{operátory posunu} (\uv{shift operators})
\begin{equation*}
\begin{split}
...
\begin{equation*}
\begin{split}
  ||A_1x||_{\ell_2}=||x||_{\ell_2}&\Rightarrow ||A_1||=\sup\limits_{||x||\leq1}||A_1 x||=1\\
  ||A_2x||_{\ell_2}=||x||_{\ell_2}&\Rightarrow ||A_2||\leq1,
\end{split}
\end{equation*}
tedy oba jsou omezené, tedy spojité, tedy $A_1,A_2\in \lin(\ell_2)$.

Přitom \begin{itemize}
    \item $A_1$ je prosté, tedy různým prvkům přiřadí různé prvky, ale není na (nic se nezobrazí např. na $(1,0,0,\dots)$
    \item $A_2$ je na, ale není prostý (rozmyslete).
\end{itemize}
}
\\
Nicméně, co se týče stability, tak i v nekonečné dimenzi platí následující hluboká věta.
\begin{theorem}[O spojitosti inverze bijektivního zobrazení]
\label{theorem:str23}
Nechť $X$ je Banachův prostor, $\map A\in \lin (X)$ je prosté a na. Potom $\map A^{-1}\in \lin (X)$, speciálně  $\map A^{-1}$ je spojité.
\end{theorem}
...
Nechť $X$ je Banachův prostor, $\map A \in \lin (X)$. Nechť dále platí \begin{enumerate}
    ...
    \item existuje $\map A^{-1}: \mathcal{R}(\map A) \to X$ (tj. $\map A$ je prostý na $X$). %TODO má tady být \to nebo \mapsto?
    \label{lemma1}
\end{enumerate}
...
\begin{itemize}
    ...
    \item Řada v b) se nazývá von Neumannova řada operátoru $(\map T-\lambda \map{Id})$ %TODO co je to von Neumannova řada? Byla definovaná už předtím, trochu ale jinak.
\end{itemize}
\end{remark}
\begin{proof}
...
\end{proof}
\Priklad{
Uvažujme $\ell_2\coloneqq \left \{ \{x_n\}_{n=1}^\infty,\; x_n\in\C,\;\sum\limits_{n=1}^\infty|x_n|^2<\infty \right \}$ prostor všech komplexních posloupností, které jsou tzv. \uv{sčítatelné s kvadrátem}. Platí, že $\ell_2$ se skalárním součinem $\innerprod{\{x_n\}}{\{y_n}\}_{\ell_2}=\sum\limits_{n=1}^\infty x_n\conj{y_n}$ (který indukuje normu $||\{x_n\}_{\ell_2}=\sqrt{\sum_{n=1}^\infty}|x_n|^2$) je úplný, a tedy Hilbertův (tj. i Banachův) prostor.

Uvažujme operátor 
\vspace*{-0.3cm}
\begin{align*}
    \map T&:\ell_2\rightarrow \ell_2\\
    \map T&:(x_1,x_2\dots,x_k,x_{k+1},\dots)\mapsto (0,x_1,x_2,\dots,x_{k-1},x_k,\dots).
\end{align*}
Protože $||\map T x ||_{\ell_2}=||x||_{\ell_2}$, je $||\map T||=\sup\limits_{||x||\leq1}||\map Tx||=\sup\limits_{||x||\leq1}||x||=1$.
...
\begin{itemize}
    \item $\lambda=0$: Už víme, že $\map T$ \uu{není na}, je prostý. Zároveň je vidět, že žádný prvek z $\ell_2$ se pomocí $\map T$ nezobrazí na $(a,0,0,\dots)$, $a\in\C,\; a\neq 0$. Nelze tedy žádnou posloupností z $\mathcal{R}(\map T)$ dokonvergovat např. k prvku $(1,0,0,\dots$). Proto $\overline{\mathcal{R}(\map T)}\neq \ell_2$, odkud plyne $0\in \Sp_R(\map T)$ (plyne z tabulky \ref{tab:spektra}). 
...
\begin{enumerate}
    ...
    \item Ukážeme, že $\map T_\lambda$ \uu{není na}, speciálně, že žádné $x\in \ell_2$ se nezobrazí na $(1,0,0,\dots)$. Nechť takové $x\in \ell_2$ existuje. Pak tedy
    \vspace*{-0.5cm}
    \begin{align*}
...
\item Mějme operátor $\map T: \ell_2\rightarrow \ell_2$
    ...
    \textcolor{gray}{\uu{Řešení:} $\map T\in \lin(\ell_2), \; \Sp(\map T)=\Sp_P(\map T=\{0\},\; \Sp_R(\map T)=\emptyset,\; \Sp_C(\map T)=\emptyset $}
...
Zkuste navíc rozmyslet, jaká je $||T||$ a zda $0\in \Sp_C(\map T)$, nebo $0\in \Sp_R(\map T)$.
    ...
    \textcolor{gray}{\uu{Řešení:} $\map T\in \lin(\ell_2), \; \Sp(\map T)=\{0\},\; \Sp_P(\map T)=\emptyset,\; \Sp_C(\map T)=\emptyset $}
\end{enumerate}
