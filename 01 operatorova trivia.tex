...
\begin{enumerate}
    ...
    \begin{itemize}
        \item \uu{operátor}: $\map T: X \to Y$
        \item \uu{funkcionál}: $\map T: X \to \K$
    \end{itemize}
    Každý funkcionál je i operátor. Budeme tedy BÚNO definovat další vlastnosti pro operátory.
    
    \item Operátor $\map T: X \to Y$ je
    \begin{itemize}
        \item \uu{lineární}: $\map T(ax + by) = a \map T(x) + b \map T(y) \hspace{2em} \forall x,y \in X \; \forall a,b \in \K$
    ...
    \begin{itemize}
        \item \uu{spojitý}: $x_n \to x \implies \map T x_n \to \map T x$ (tzv. „Heineova definice“)
...
Buďte $X,Y$ Banachovy. Pokud je $\map T: X \to Y$ \uu{lineární} operátor, pak
$$
...
Buď $\map T: X \to Y$, $X,Y$ Banachovy, $T$ lineární, \uu{$\dim X < \infty$}. Potom $\map T$ je omezený, a tedy i spojitý.
\end{theorem}
\begin{proof}
$$
    x \in X
...
\Poznamka V konečné dimenzi jsou tedy všechny lineární operátory už spojité. Přirozená otázka: platí to i pro $\dim X = \infty$? Ne: $X, Y$ NLP, $\dim X = \infty$, pak $\exists \map T: X \to Y$ lineární a neomezený. (V případě $X,Y$ Banachovy tedy i nespojitý).
\\ \\
\Priklad{
